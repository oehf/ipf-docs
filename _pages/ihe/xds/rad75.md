---
title: xcai-rad75 component
layout: single
permalink: /docs/ihe/rad75/
toc: true
toc_icon: align-left
toc_sticky: true
---

{% assign tx = site.data.ihe["rad75"] %}
...
{% include figure image_path="/assets/images/rad75.svg" alt="RAD-75 actors" caption="RAD-75 transaction and actors " %}
...
```
{{ tx.component }}:serviceName?homeCommunityId=<homeCommunityId>[&parameters]
```
...
`http://eHealth.server.org:8888/IHE/xcai/{{ tx.link }}`
...
```java
    from("{{ tx.component }}:{{ tx.link }}?homeCommunityId=1.2.3.4.5&audit=true")
      .process(myProcessor)
